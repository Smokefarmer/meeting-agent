...
const ConfigSchema = z.object({
  ...
  openclawGatewayPort: z.number().int().min(1).max(65535).default(18789),
  openclawHooksToken: z.string().min(1).default('meetingclaw-internal'),
...
export function loadConfig(): OpenClawConfig {
  return ConfigSchema.parse({
    ...
    openclawGatewayPort: parseInt(process.env.OPENCLAW_GATEWAY_PORT ?? '18789', 10),
    openclawHooksToken: process.env.OPENCLAW_HOOKS_TOKEN ?? 'meetingclaw-internal',
